...
거래량 상위 종목 데이터와 시장 환경을 종합 분석하여 단기 수익이 기대되는 종목을 엄선해주세요.
...
시장 환경별 전략 (시장 지수 정보가 있으면 반드시 참고):
- 상승장 (지수 +0.5% 이상): 모멘텀 종목 적극 선별
- 보합장 (지수 -0.5% ~ +0.5%): 거래량 급증 + 개별 모멘텀 종목 선별
- 하락장 (지수 -0.5% ~ -1.5%): 방어주, 역행 상승 종목, 인버스 ETF 검토
- 급락장 (지수 -1.5% 이하): 인버스 ETF(예: KODEX 인버스, KODEX 200선물인버스2X) 우선, 일반 종목은 최소화

절대 제외 대상:
- 등락률 +7% 이상 (고점 추격 매수 위험)
- 등락률 -5% 이하 (하락 추세) — 단, 인버스 ETF는 예외
- 관리종목, 투기성 테마주 의심 종목

반드시 아래 JSON 형식으로만 응답하세요:
{"picks": [{"symbol": "종목코드", "reason": "선별 사유"}], "market_sentiment": "bullish 또는 bearish 또는 neutral", "summary": "시장 분위기 한줄 요약"}

최대 7개 종목을 선별하세요. 적극적으로 기회를 찾되, 위 제외 조건은 반드시 지키세요. 유망 종목이 없으면 빈 리스트를 반환하세요."""
...
- 하락장 전환 시: 일반 롱 종목은 적극적으로 drops 처리, 인버스 ETF로 교체 검토

시장 환경별 전략 (시장 지수 정보가 있으면 반드시 참고):
- 하락장 (지수 -0.5% 이하): 일반 종목 비중 축소, 인버스 ETF(KODEX 인버스 등) 추천
- 급락장 (지수 -1.5% 이하): 일반 롱 종목 전량 drops 권장, 인버스 ETF 우선 picks

신규 종목 선별 기준:
- 거래량 급증 + 등락률 +1~4% (상승 초기)
- 등락률 +7% 이상 급등 종목은 제외 (고점 매수 위험)
- 등락률 -5% 이하 급락 종목은 제외 (하락 추세) — 단, 인버스 ETF는 예외

반드시 아래 JSON 형식으로만 응답하세요:
{"picks": [{"symbol": "종목코드", "reason": "선별 사유"}], "drops": ["제거할종목코드1"], "market_sentiment": "bullish 또는 bearish 또는 neutral", "summary": "시장 분위기 한줄 요약"}

picks는 최대 7개. 적극적으로 신규 기회를 찾아주세요. drops는 손실 종목이나 모멘텀 상실 종목만 포함. 보유 종목 현황이 있으면 반드시 참고하세요."""
...
class StockScanner:
    ...
    def set_market_context(self, context: str):
        """시장 지수 컨텍스트를 설정한다 (스캔/로테이션 프롬프트에 포함)."""
    ...
    def scan_us_and_select(self, candidates: list[str] | None = None, rotate: bool = False) -> list[dict]:
        ...
        if self._market_context:
            lines.append(self._market_context)
            lines.append("")
        lines.append("[미국 주요 종목 시세]")
